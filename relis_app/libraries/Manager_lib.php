...
class Manager_lib
{
	...
	function get_left_menu_screen(){
		...
		if(is_project_creator(project_db()) OR has_usergroup(1) OR get_appconfig_element('screening_result_on') ){
			...
			$menu['general']['menu']['papers']['sub_menu']['all_papers']=array( 'label'=>'All papers', 'url'=>'relis/manager/list_screen/screen_paper', '');
	...
	function get_left_menu_admin(){
		...
		if(has_usergroup(1)){
		$menu['general']['menu']['users']=array('label'=>'Users','url'=>'manager/entity_list/users','icon'=>'user');
		$menu['general']['menu']['usergroup']=array('label'=>'User groups','url'=>'manager/entity_list/usergroup','icon'=>'users');
