...
if(isset($_POST['submit_form'])){
	...
	if(empty($_POST['user_password'])){
		...
		if(trim($_POST['user_password']) != trim($_POST['user_password_v'])  ){
			array_push($error, "Wrong Password validation");
		}
	}
		...
		if (!$link) {
			...
			if (mysql_query($sql, $link)) {
				...
				if (!$db_selected) {
					...
					$sql="INSERT INTO relis_users (user_name,user_username,user_password,user_mail,user_usergroup) VALUES('".$full_name."','".$user_name."','".$user_password."','".$user_mail."',1)";
					$result = mysql_query($sql);
					if (!$result) {
						die('Invalid query : ' . mysql_error());
					}
					...
					//echo "<h2>Install done</h2>";
		
					//echo "<h2><a href='../index.php'>Launch application</a></h2>";
					install_result();
				}
...
$sess_folder=str_replace('/install/run_install.php','',$actual_folder).'/cside/sessions';
$sess_folder=str_replace('/install/index.php','',$sess_folder).'/cside/sessions';
$sess_folder=str_replace('/install/','',$sess_folder).'/cside/sessions';


//echo $actual_link."<br/>";
//echo $actual_folder."<br/>";
...
$config[\'sess_save_path\'] = "'.$sess_folder.'";
 ';
